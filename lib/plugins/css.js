(function() {
  ...
  StylesheetsPackage = (function(_super) {
    ...
    StylesheetsPackage.types = ['stylesheets', 'css'];

    StylesheetsPackage["default"] = 'css';

    function StylesheetsPackage(options, sources, vendor) {
      var bundles, lib, _i, _len, _ref3;
      StylesheetsPackage.__super__.constructor.apply(this, arguments);
      _.defaults(options, {
        compress: true
      });
      this.compress = options.compress, this.build = options.build, bundles = options.bundles;
    ...
    StylesheetsPackage.prototype.compressedPath = function(file) {
      return path.join(this.build, compress === true ? util.changeext(file.relpath, '.min.css') : _.template(compress)({
        filename: file.relpath
      }));
    };

    StylesheetsPackage.prototype.compress = function(original, dest, cb) {
      ({
