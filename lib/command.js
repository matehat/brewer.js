(function() {
  ...
  this.run = function(argv) {
    ...
    program.command('watch').description("Watch for modifications on source files, automatically\ncompiling/compressing/packaging when they occur.");
    program.command('server [port]').description("Start the " + (clr('Brewer', 'green')) + " server").action(function(port) {
      return (require('./web')).run(port != null ? port : 3000);
    });
