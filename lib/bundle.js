(function() {
  var Bundle, debug, finished, fs, path, util, _ref,
    __slice = Array.prototype.slice;

  path = require('path');

  util = require('./util');

  _ref = require('./command'), finished = _ref.finished, debug = _ref.debug;

  fs = require('fs');

  this.Bundle = Bundle = (function() {

    function Bundle(package, file) {
      this.package = package;
      this.file = file;
      this.basepath = path.join(this.package.build, this.file);
      this.compressedFile = this.package.compressedFile({
        filename: util.changeExtension(this.basepath, '')
      });
    }

    Bundle.prototype.buildPath = function() {
      var _ref2;
      if (this._buildPath == null) {
        this._buildPath = util.changeExtension(this.basepath, (_ref2 = this.constructor.buildext) != null ? _ref2 : this.constructor.ext);
      }
      return this._buildPath;
    };

    Bundle.prototype.sourcePath = function(i) {
      return this.package.fullPath(i < this.files.length ? this.files[i] : this.file);
    };

    Bundle.prototype.sourcePaths = function() {
      var i, _ref2, _results;
      _results = [];
      for (i = 0, _ref2 = this.files.length; 0 <= _ref2 ? i <= _ref2 : i >= _ref2; 0 <= _ref2 ? i++ : i--) {
        _results.push(this.sourcePath(i));
      }
      return _results;
    };

    Bundle.prototype.readFile = function(i, cb, mod) {
      var rs,
        _this = this;
      if (mod == null) {
        mod = (function(a) {
          return a;
        });
      }
      return rs = fs.readFile(this.sourcePath(i), {
        encoding: 'utf-8'
      }, function(err, data) {
        if (err != null) throw err;
        _this.stream += mod(data.toString());
        return _this.nextFile(i, cb, mod);
      });
    };

    Bundle.prototype.nextFile = function(i, cb, mod) {
      if (i < this.files.length) {
        return this.readFile(i + 1, cb, mod);
      } else {
        cb(this.stream);
        return delete this.stream;
      }
    };

    Bundle.prototype.bundle = function(cb, unchanged) {
      var buildPath,
        _this = this;
      util.makedirs(path.dirname((buildPath = this.buildPath())));
      return this.package.deps(this.file, function(files) {
        _this.files = files;
        return util.newest.apply(util, [buildPath].concat(__slice.call(_this.sourcePaths()), [function(newest) {
          if (newest) {
            return cb(buildPath);
          } else {
            _this.stream = '';
            return _this.readFile(0, function(data) {
              return _this.convertFile(data, function(newdata) {
                return fs.writeFile(buildPath, newdata, 'utf-8', function() {
                  finished('Packaged', buildPath);
                  return cb(buildPath);
                });
              });
            });
          }
        }]));
      });
    };

    Bundle.prototype.compress = function(cb) {
      var buildPath, cmpFile,
        _this = this;
      return util.newer((cmpFile = this.compressedFile), (buildPath = this.buildPath()), function(err, newer) {
        if (newer) return cb(cmpFile);
        return fs.readFile(buildPath, 'utf-8', function(err, data) {
          return _this.compressFile(data, function(code) {
            return fs.writeFile(cmpFile, code, 'utf-8', function() {
              finished('Compressed', cmpFile);
              return cb(cmpFile);
            });
          });
        });
      });
    };

    Bundle.prototype.convertFile = function(data, cb) {
      return cb(data);
    };

    Bundle.prototype.compressFile = Bundle.prototype.convertFile;

    return Bundle;

  })();

}).call(this);
