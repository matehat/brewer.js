(function() {
  ...
  Package = (function(_super) {
    ...
    Package.prototype.file = function(relpath, type, fullpath, src) {
      ...
      debug('%%%', 'file:', this.name, file.fullpath, file.relpath, file.type);
      return file;
    };

    Package.prototype.actualize = function(cb) {
      var allFiles, f, file, files, i, iter, leaves, relpath, type, _i, _len, _ref2;
      debug('Actualizing', this.name);
      allFiles = [];
      _ref2 = this.files;
      for (type in _ref2) {
        files = _ref2[type];
    ...
    Package.prototype.registerSource = function(src) {
      var type, _base, _i, _len, _ref2, _ref3,
        _this = this;
      _ref2 = ['all', 'loading'].concat(__slice.call(src.constructor.types));
      for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
        type = _ref2[_i];
        ((_ref3 = (_base = this.sources)[type]) != null ? _ref3 : _base[type] = []).push(src);
      }
      return src.files(null, function(files) {
        _this.sources.loading = _.without(_this.sources.loading, src);
        if (_this.sources.loading.length === 0) return _this.emit('ready');
      });
    };
